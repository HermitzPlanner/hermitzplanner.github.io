...
async function amogus() {
  ...
  eventData.forEach(function (eventJson) {
    ...
    events.innerHTML += `
        <a href="#skin-${eventJson.eventcode}" id="link-${eventJson.eventcode}" style="all: unset; width: 100%;">
          <button id="event-${eventJson.eventcode}">
            <div class="event-row flex">
              <div class="event-n flex">${eventCounter}</div>
              <div class="event-mid flex" style="background-image: linear-gradient(rgba(13, 13, 13, 0.82), rgba(3, 3, 3, 0.90)), url('events/resized_${eventJson.eventcode}.jpg');">${eventJson.event}</div>
              <div class="event-info flex-column">
                <div class="info-container flex">
                  <div class="info-icon flex"><img class="puregem" src="puregem.png"></div>
                ...
                <div class="info-container flex" >
                  <div class="info-icon flex">=</div>
                  <div class="info-value flex total-value">${totalValue}</div>
                </div>
              </div>
            </div>
          </button>
  ...
  const eventMid = document.querySelectorAll('.event-mid')
  ...
  eventButtons.forEach(button => {
    button.addEventListener('click', function () {
      ...
      // CHANGE STYLE OF SELECTED EVENTS
      eventMid.forEach(event => {
        // console.log(event.parentElement.parentElement)
        event.style.color = 'white';
        event.style.transition = '0.3s';
        event.style.backgroundImage = `linear-gradient(rgba(13, 13, 13, 0.80), rgba(3, 3, 3, 0.85)), url("events/resized_${event.parentElement.parentElement.id.replace('event-', '')}.jpg"`
      });
      // console.log(button.id.replace('event-', '')) 
      button.children[0].children[1].style.color = `transparent` 
      button.children[0].children[1].style.transition = '0.3s';
      button.children[0].children[1].style.backgroundImage = `linear-gradient(rgba(13, 13, 13, 0.0), rgba(3, 3, 3, 0.0)), url("events/resized_${button.id.replace('event-', '')}.jpg"`
      ...
      eventRow.forEach(event => {
        event.style.height = '188px';
        event.style.transition = '0.3s';
      });
      
      childDiv.style.height = '188px';
